...
<article class="user-details">
    ...
    <section class="top-part">
        <div class="single-chart">
            <h4>Dodał łącznie: {{ $user -> words }} słów <span class="gray" style="color: #c4c4c4">({{ floor($user -> words / count(\App\Word::all())*100) }}%)</span></h4>
            @if ($data)
                <div id="chart1"></div>
        ...
        <div class="single-chart">
            <h4>Uzyskał łącznie: {{ $user -> exercises -> matching + $user -> exercises -> writing }} punktów z ćwiczeń</h4>
            @if ($data)
                <div id="chart2"></div>
                <p>Ostatnie 3 tygodnie</p>
            @else
        ...
        <div class="single-chart">
            <h4>Powtórzył łącznie: {{ $user -> exercises -> translation }} słów <span class="gray" style="color: #c4c4c4">({{ floor($user -> exercises -> translation / count(\App\Word::all())*100) }}%)</span></h4>
            @if ($data)
                <div id="chart3"></div>
                <p>Ostatnie 3 tygodnie</p>
            @else
...
@if ($data)
    ...
    <script>
        ...
        el: '#chart1',
        url: "http://localhost:8000/charts/user/1/" + {{$user -> id}},
        ...
        el: '#chart2',
        url: "http://localhost:8000/charts/user/2/" + {{$user -> id}},
        hooks: new ChartisanHooks()
            .legend(false)
            .beginAtZero()
            .minimalist()
        ...
        el: '#chart3',
        url: "http://localhost:8000/charts/user/3/" + {{$user -> id}},
        hooks: new ChartisanHooks()
            .legend(false)
            .beginAtZero()
            .minimalist()
